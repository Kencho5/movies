...
import { SeasonResponse, TitleResponse } from "@core/interfaces/title";
...
export class MovieService {
  ...
  getMovies(page: number = 1, filters?: string): Observable<any> {
  ...
  getMovie(id: string): Observable<TitleResponse> {
    return this.http.get<TitleResponse>(apiUrl(`titles/${id}?loader=titlePage`));
  }

  getSeason(titleId: string, seasonNumber: number): Observable<SeasonResponse> {
    return this.http.get<SeasonResponse>(
      apiUrl(`titles/${titleId}/seasons/${seasonNumber}?loader=seasonPage`)
    );
  }
